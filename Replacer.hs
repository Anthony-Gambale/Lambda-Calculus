...
replace env expr = case expr of
    Atom a             -> env (Atom a)
    Lambda (Atom a) e  -> Lambda (Atom a) (replace (\x -> if x == (Atom a) then (Atom a) else env x) e)
    Apply e1 e2        -> Apply (replace env e1) (replace env e2)
    Let (Atom a) e1 e2 -> let newe1 = replace env e1
                           in Let (Atom a) newe1 (replace (\x -> if x == (Atom a) then newe1 else env x) e2)
